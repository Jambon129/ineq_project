...
An interesting fact is, that in 2017 pre-tax factor and pre-tax national income decreased but post-tax disposable income increased, which seems to lead to the conclusion that income was falling from 2016 to 2017 but redistributive measures still ensured an increase in post-tax disposable income.
...
print.xtable(xtable(mean_p1noyear,align='lccc', caption='Mean yearly income in 2015 PLN for all three income concepts for P1 (EU-SILC 2018, Eurostat 2019).', digits=2, label='mean_p1'), booktabs = TRUE)
@
...
plot_meanp1 <- ggplot(mean_p1, aes(x=year, y=PTDI_clean), color='mean_p1')+
  geom_hline(yintercept=5000, color = "black", size=0.25)+
  geom_line(aes(y=mean_p1$PTFI_clean), color='dark blue', size=0.5) +
  geom_point(aes(y=mean_p1$PTFI_clean), color='dark blue', size=1) +
  geom_line(aes(y=mean_p1$PTNI_clean), color='blue', size=0.5) +
  geom_point(aes(y=mean_p1$PTNI_clean),color=' blue', size=1) +
  geom_line(aes(y=mean_p1$PTDI_clean), color='light blue', size=0.5) +
  geom_point(aes(y=mean_p1$PTDI_clean), color='light blue', size=1) +
  scale_x_continuous(breaks=mean_p1$year) +
  labs(x = "Year", 
       y = "Yearly income in 2015 PLN",
       caption='EU-SILC 2018, Eurostat 2019')+theme_minimal()#+scale_linetype_manual(name="Title of legend", values = c('mean_p1$pretaxfacinc', 'mean_p1$pretaxnational', 'mean_p1$posttax'), guide = guide_legend(override.aes = list(color = c('blue', 'red','green')))) 
plot_meanp1
@
...
An interesting fact is that the median post-tax disposable income decreased like our other income measures in 2017 but the mean showed an increase. This implies that redistributive measures favoured high incomes in this year more.
...
print.xtable(xtable(median_p1noyear,align='lccc', caption='Median yearly income in 2015 PLN for all three income concepts for P1 (EU-SILC 2018, Eurostat 2019).', digits=2, label='median_p1'), booktabs = TRUE)
@
<<figure_medianp1, cache=FALSE, echo=FALSE, fig.cap='Median income in 2015 PLN for the three income concepts over time for P1. The graphs for pre-tax factor income, pre-tax national income and post-tax disposable income are again dark blue, blue and light blue, respectively.', fig.align='center', fig.width=5, fig.height=4>>=
plot_medianp1 <- ggplot(median_p1, aes(x=year, y=PTDI_clean))+
  geom_hline(yintercept=5000, color = "black", size=0.25)+
  geom_line(aes(y=median_p1$PTFI_clean), color='dark blue', size=0.5) +
  geom_point(aes(y=median_p1$PTFI_clean), color='dark blue', size=1) +
  geom_line(aes(y=median_p1$PTNI_clean), color='blue', size=0.5) +
  geom_point(aes(y=median_p1$PTNI_clean),color=' blue', size=1) +
  geom_line(aes(y=median_p1$PTDI_clean), color='light blue', size=0.5) +
  geom_point(aes(y=median_p1$PTDI_clean), color='light blue', size=1) +
  scale_x_continuous(breaks=median_p1$year) +
  labs(x = "Year", 
       y = "Yearly income in 2015 PLN",
       caption='EU-SILC 2018, Eurostat 2019')+theme_minimal()#+scale_linetype_manual(name="Title of legend", values = c('mean_p2$pretaxfacinc', 'mean_p2$pretaxnational', 'mean_p2$posttax'), guide = guide_legend(override.aes = list(color = c('blue', 'red','green')))) 
plot_medianp1
@
...
mean_median17 <- ggplot(densframe_p1, aes (x = dens, y= ..density..)) +
  ...
  labs(x = "Yearly Income in 2015 PLN ", 
       y = "Density",
...
plot_s80s20p1 <- ggplot(s80_s20p1, aes(x=year, y=PTDI_clean))+
  geom_hline(yintercept=0.0, color = "black", size=0.25)+
  geom_line(aes(y=s80_s20p1$PTFI_clean), color='dark blue', size=0.5) +
  geom_point(aes(y=s80_s20p1$PTFI_clean), color='dark blue', size=1) +
  geom_line(aes(y=s80_s20p1$PTNI_clean), color='blue', size=0.5) +
  geom_point(aes(y=s80_s20p1$PTNI_clean),color=' blue', size=1) +
  geom_line(aes(y=s80_s20p1$PTDI_clean), color='light blue', size=0.5) +
  geom_point(aes(y=s80_s20p1$PTDI_clean), color='light blue', size=1) +
  scale_x_continuous(breaks=s80_s20p1$year) +
  labs(x = "Year", 
       y = "S80/S20 for Poland over time",
       caption='EU-SILC 2018, Eurostat 2019')+theme_minimal()#+scale_linetype_manual(name="Title of legend", values = c('mean_p2$pretaxfacinc', 'mean_p2$pretaxnational', 'mean_p2$posttax'), guide = guide_legend(override.aes = list(color = c('blue', 'red','green')))) 
plot_s80s20p1
@
...
top10p1total<-svyby(~PTFI_clean+PTNI_clean+PTDI_clean, by=~year, design=P1.svy, FUN=svytotal)
top10p1total<-rename(top10p1total, 'PTFI_total'='PTFI_clean', 'PTNI_total'='PTNI_clean', 'PTDI_total'='PTDI_clean')
top10p1totalnoyear<-top10p1total[,2:4]

top10p1upperPTFI<-svyby(~PTFI_clean, by=~year, design = subset(P1.svy, PTFI_clean>=as.numeric(svyquantile(~PTFI_clean, P1.svy, quantile=c(.9)))), FUN=svytotal)
top10p1upperPTNI<-svyby(~PTNI_clean, by=~year, design = subset(P1.svy, PTNI_clean>=as.numeric(svyquantile(~PTNI_clean, P1.svy, quantile=c(.9)))), FUN=svytotal)
top10p1upperPTDI<-svyby(~PTDI_clean, by=~year, design = subset(P1.svy, PTDI_clean>=as.numeric(svyquantile(~PTDI_clean, P1.svy, quantile=c(.9)))), FUN=svytotal)

top10p1upperPTNI<-top10p1upperPTNI[,2]
top10p1upperPTDI<-top10p1upperPTDI[,2]

top10p1upper<-cbind(top10p1upperPTFI, top10p1upperPTNI, top10p1upperPTDI)
top10p1upper<-rename(top10p1upper, 'PTFI_upper'='PTFI_clean', 'PTNI_upper'='top10p1upperPTNI', 'PTDI_upper'='top10p1upperPTDI')
top10p1upper<-select(top10p1upper, year, PTFI_upper, PTNI_upper, PTDI_upper)
top10p1uppernoyear<-top10p1upper[,2:4]

top10p1_share<-top10p1uppernoyear
top10p1_share[,1]<-top10p1_share[,1]/top10p1totalnoyear[,1]
top10p1_share[,2]<-top10p1_share[,2]/top10p1totalnoyear[,2]
top10p1_share[,3]<-top10p1_share[,3]/top10p1totalnoyear[,3]

top10p1year<-top10p1upper
top10p1year[,2]<-top10p1year[,2]/top10p1totalnoyear[,1]
top10p1year[,3]<-top10p1year[,3]/top10p1totalnoyear[,2]
top10p1year[,4]<-top10p1year[,4]/top10p1totalnoyear[,3]


names(top10p1_share)<-c('Pre-tax factor', 'Pre-tax national', 'Post-tax disposable')

print.xtable(xtable(top10p1_share,align='lccc', caption='The total-income share of the top 10 percent over time for all three income concepts for P1 (EU-SILC 2018, Eurostat 2019).', digits=2, label='top10p1'), booktabs = TRUE)
...
plot_top10p1 <- ggplot(top10p1year, aes(x=year, y=PTDI_upper))+
  geom_hline(yintercept=0.25, color = "black", size=0.25)+
  geom_line(aes(y=top10p1year$PTFI_upper), color='dark blue', size=0.5) +
  geom_point(aes(y=top10p1year$PTFI_upper), color='dark blue', size=1) +
  geom_line(aes(y=top10p1year$PTNI_upper), color='blue', size=0.5) +
  geom_point(aes(y=top10p1year$PTNI_upper),color=' blue', size=1) +
  geom_line(aes(y=top10p1year$PTDI_upper), color='light blue', size=0.5) +
  geom_point(aes(y=top10p1year$PTDI_upper), color='light blue', size=1) +
  scale_x_continuous(breaks=top10p1year$year) +
  labs(x = "Year", 
       y = "Top 10 percent share for Poland over time",
       caption='EU-SILC 2018, Eurostat 2019')+theme_minimal()#+scale_linetype_manual(name="Title of legend", values = c('mean_p2$pretaxfacinc', 'mean_p2$pretaxnational', 'mean_p2$posttax'), guide = guide_legend(override.aes = list(color = c('blue', 'red','green')))) 
plot_top10p1
@
...
@
Our analysis shows indeed a very large university wage premium, especially for men: A man with tertiary education had in 2005 a pre-tax factor income 6,586 PLN higher than a man of the same age with no education at all, and still 4,420 PLN more than a man of same age with a post-secondary education. For women, the respective values are 3,819 PLN and 2,638 PLN. The redistributive system is able to temper these extreme differences among educational groups (the respective values regarding the post-tax disposable income for men are 6,107 PLN and 3,904 PLN, and for women 3,400 PLN and 1,922 PLN), but they remain high. Also, there is a very high difference between men and women both before and after redistribution by the tax and social transfer system.

In 2013, the university wage premium relative to the intercept regarding the pre-tax factor income for men has slightly (6457 PLN) decreased, but regarding post-tax disposable income, it has even increased (6716 PLN). Same applies to women: The university wage premium relative to the intercept has decreased for pre-tax factor income (3177 PLN), but increased for post-tax disposable income (3469 PLN). One explanation for this phenomenom might be the poor redistributive performance of the Polish income tax system, which only has one progression step \parencite{pwc}. Of course, we also have to keep in mind that for P2 there is no individual taxation of incomes, presenting these premia in a very drastical light. Nonetheless, it has become clear that income inequality in Poland is indeed driven by gender and educational premia. Regarding the latter, this is in line with \cite{gomulka2016poland}, who states:
...
print.xtable(xtable(mean_p2noyear,align='lccc', caption='Mean yearly income in 2015 PLN for all three income concepts for P2 (EU-SILC 2018, Eurostat 2019).', digits=2, label='mean_p2'), booktabs = TRUE)
@
...
plot_meanp2 <- ggplot(mean_p2, aes(x=year, y=posttax))+
  geom_hline(yintercept=5000, color = "black", size=0.25)+
  geom_line(aes(y=mean_p2$PTFI_clean), color='dark blue', size=0.5) +
  geom_point(aes(y=mean_p2$PTFI_clean), color='dark blue', size=1) +
  geom_line(aes(y=mean_p2$PTNI_clean), color='blue', size=0.5) +
  geom_point(aes(y=mean_p2$PTNI_clean),color=' blue', size=1) +
  geom_line(aes(y=mean_p2$PTDI_clean), color='light blue', size=0.5) +
  geom_point(aes(y=mean_p2$PTDI_clean), color='light blue', size=1) +
  scale_x_continuous(breaks=mean_p2$year) +
  labs(x = "Year", 
       y = "Yearly income in 2015 PLN",
       caption='EU-SILC 2018, Eurostat 2019')+theme_minimal()#+scale_linetype_manual(name="Title of legend", values = c('mean_p2$pretaxfacinc', 'mean_p2$pretaxnational', 'mean_p2$posttax'), guide = guide_legend(override.aes = list(color = c('blue', 'red','green')))) 
plot_meanp2
...
median_p2noyear<-median_p1[,2:4]
names(median_p2noyear)<-c('Pre-tax factor', 'Pre-Tax National', 'Post-Tax Disposable')
print.xtable(xtable(median_p1noyear,align='lccc', caption='Median yearly income in 2015 PLN for all three income concepts for P2 (EU-SILC 2018, Eurostat 2019).', digits=2, label='median_p2'), booktabs = TRUE)
@

<<figure_medianp2, echo=FALSE, fig.cap='Median income in 2015 PLN for the three income concepts over time for P2. Pre-tax factor income is again dark blue, pre-tax national income is blue and post-tax disposable income is again light blue.', fig.align='center', fig.width=5, fig.height=4, fig.pos='H'>>=
plot_medianp2 <- ggplot(median_p2, aes(x=year, y=posttax))+
  geom_hline(yintercept=5000, color = "black", size=0.25)+
  geom_line(aes(y=median_p2$PTFI_clean), color='dark blue', size=0.5) +
  geom_point(aes(y=median_p2$PTFI_clean), color='dark blue', size=1) +
  geom_line(aes(y=median_p2$PTNI_clean), color='blue', size=0.5) +
  geom_point(aes(y=median_p2$PTNI_clean),color=' blue', size=1) +
  geom_line(aes(y=median_p2$PTDI_clean), color='light blue', size=0.5) +
  geom_point(aes(y=median_p2$PTDI_clean), color='light blue', size=1) +
  scale_x_continuous(breaks=median_p1$year) +
  labs(x = "Year", 
       y = "Yearly income in 2015 PLN",
       caption='EU-SILC 2018, Eurostat 2019')+theme_minimal()#+scale_linetype_manual(name="Title of legend", values = c('mean_p2$pretaxfacinc', 'mean_p2$pretaxnational', 'mean_p2$posttax'), guide = guide_legend(override.aes = list(color = c('blue', 'red','green')))) 
...
plot_ginip2 <- ggplot(gini_p2, aes(x=year, y=PTDI_clean))+
  geom_hline(yintercept=0.50, color = "black", size=0.25)+
  geom_line(aes(y=gini_p2$PTFI_clean), color='dark blue', size=0.5) +
  geom_point(aes(y=gini_p2$PTFI_clean), color='dark blue', size=1) +
  geom_line(aes(y=gini_p2$PTNI_clean), color='blue', size=0.5) +
  geom_point(aes(y=gini_p2$PTNI_clean),color=' blue', size=1) +
  geom_line(aes(y=gini_p2$PTDI_clean), color='light blue', size=0.5) +
  geom_point(aes(y=gini_p2$PTDI_clean), color='light blue', size=1) +
  scale_x_continuous(breaks=gini_p2$year) +
  labs(x = "Year", 
       y = "Gini coefficient for Poland over time",
       caption='EU-SILC 2018, Eurostat 2019')+theme_minimal()#+scale_linetype_manual(name="Title of legend", values = c('mean_p2$pretaxfacinc', 'mean_p2$pretaxnational', 'mean_p2$posttax'), guide = guide_legend(override.aes = list(color = c('blue', 'red','green')))) 
plot_ginip2
...
plot_s80s20p2 <- ggplot(s80_s20p2, aes(x=year, y=PTDI_clean))+
  geom_hline(yintercept=0.0, color = "black", size=0.25)+
  geom_line(aes(y=s80_s20p2$PTFI_clean), color='dark blue', size=0.5) +
  geom_point(aes(y=s80_s20p2$PTFI_clean), color='dark blue', size=1) +
  geom_line(aes(y=s80_s20p2$PTNI_clean), color='blue', size=0.5) +
  geom_point(aes(y=s80_s20p2$PTNI_clean),color=' blue', size=1) +
  geom_line(aes(y=s80_s20p2$PTDI_clean), color='light blue', size=0.5) +
  geom_point(aes(y=s80_s20p2$PTDI_clean), color='light blue', size=1) +
  scale_x_continuous(breaks=s80_s20p2$year) +
  labs(x = "Year", 
       y = "S80/S20 for Poland over time",
       caption='EU-SILC 2018, Eurostat 2019')+theme_minimal()#+scale_linetype_manual(name="Title of legend", values = c('mean_p2$pretaxfacinc', 'mean_p2$pretaxnational', 'mean_p2$posttax'), guide = guide_legend(override.aes = list(color = c('blue', 'red','green')))) 
plot_s80s20p2
...
top10p2total<-svyby(~PTFI_clean+PTNI_clean+PTDI_clean, by=~year, design=P2.svy, FUN=svytotal)
top10p2total<-rename(top10p2total, 'PTFI_total'='PTFI_clean', 'PTNI_total'='PTNI_clean', 'PTDI_total'='PTDI_clean')
top10p2totalnoyear<-top10p2total[,2:4]

top10p2upperPTFI<-svyby(~PTFI_clean, by=~year, design = subset(P2.svy, PTFI_clean>=as.numeric(svyquantile(~PTFI_clean, P2.svy, quantile=c(.9)))), FUN=svytotal)
top10p2upperPTNI<-svyby(~PTNI_clean, by=~year, design = subset(P2.svy, PTNI_clean>=as.numeric(svyquantile(~PTNI_clean, P2.svy, quantile=c(.9)))), FUN=svytotal)
top10p2upperPTDI<-svyby(~PTDI_clean, by=~year, design = subset(P2.svy, PTDI_clean>=as.numeric(svyquantile(~PTDI_clean, P2.svy, quantile=c(.9)))), FUN=svytotal)

top10p2upperPTNI<-top10p2upperPTNI[,2]
top10p2upperPTDI<-top10p2upperPTDI[,2]

top10p2upper<-cbind(top10p2upperPTFI, top10p2upperPTNI, top10p2upperPTDI)
top10p2upper<-rename(top10p2upper, 'PTFI_upper'='PTFI_clean', 'PTNI_upper'='top10p2upperPTNI', 'PTDI_upper'='top10p2upperPTDI')
top10p2upper<-select(top10p2upper, year, PTFI_upper, PTNI_upper, PTDI_upper)
top10p2uppernoyear<-top10p2upper[,2:4]

top10p2_share<-top10p2uppernoyear
top10p2_share[,1]<-top10p2_share[,1]/top10p2totalnoyear[,1]
top10p2_share[,2]<-top10p2_share[,2]/top10p2totalnoyear[,2]
top10p2_share[,3]<-top10p2_share[,3]/top10p2totalnoyear[,3]

top10p2year<-top10p2upper
top10p2year[,2]<-top10p2year[,2]/top10p2totalnoyear[,1]
top10p2year[,3]<-top10p2year[,3]/top10p2totalnoyear[,2]
top10p2year[,4]<-top10p2year[,4]/top10p2totalnoyear[,3]
...
plot_top10p2 <- ggplot(top10p2year, aes(x=year, y=PTDI_upper))+
  geom_hline(yintercept=0.25, color = "black", size=0.25)+
  geom_line(aes(y=top10p2year$PTFI_upper), color='dark blue', size=0.5) +
  geom_point(aes(y=top10p2year$PTFI_upper), color='dark blue', size=1) +
  geom_line(aes(y=top10p2year$PTNI_upper), color='blue', size=0.5) +
  geom_point(aes(y=top10p2year$PTNI_upper),color=' blue', size=1) +
  geom_line(aes(y=top10p2year$PTDI_upper), color='light blue', size=0.5) +
  geom_point(aes(y=top10p2year$PTDI_upper), color='light blue', size=1) +
  scale_x_continuous(breaks=top10p2year$year) +
  labs(x = "Year", 
       y = "Top 10 percent share for Poland over time",
       caption='EU-SILC 2018, Eurostat 2019')+theme_minimal()#+scale_linetype_manual(name="Title of legend", values = c('mean_p2$pretaxfacinc', 'mean_p2$pretaxnational', 'mean_p2$posttax'), guide = guide_legend(override.aes = list(color = c('blue', 'red','green')))) 
plot_top10p2
