...
<<get_data, eval=TRUE, cache=FALSE, echo=TRUE, message=FALSE, warning=FALSE>>=
library(dplyr)
library(ggplot2)
library(survey)
library(convey)
...
plot_meanp1 <- ggplot(meltmean_p1, aes(x=year, y=value, color=variable))+
  ...
  labs(title='Mean Income for P1',
    x = "Year", 
       ...
       caption='EU-SILC 2018, Eurostat 2019', color='')+theme_minimal() + 
  scale_color_manual(values=c("#00008B", "#0000FF", "#ADD8E6"), labels=c('Pre-tax factor income', 'Pre-tax national income', 'Post-tax disposable income'))+theme(legend.position = 'bottom') #+ scale_fill_manual(name="Income Concepts", labels=c("Control", "Treatment 1", "Treatment 2")) #+scale_linetype_manual(name="Title of legend", values = c('mean_p1$pretaxfacinc', 'mean_p1$pretaxnational', 'mean_p1$posttax'), guide = guide_legend(override.aes = list(color = c('blue', 'red','green')))) 
plot_meanp1
@
...
plot_medianp1 <-  ggplot(meltmedian_p1, aes(x=year, y=value, color=variable))+
  ...
  labs(title='Median Income for P1', x = "Year", 
       y = "Yearly income in 2015 EUR",
       caption='EU-SILC 2018, Eurostat 2019', color='')+theme_minimal() + 
  scale_color_manual(values=c("#00008B", "#0000FF", "#ADD8E6"), labels=c('Pre-tax factor income', 'Pre-tax national income', 'Post-tax disposable income'))+theme(legend.position = 'bottom')
plot_medianp1
@
