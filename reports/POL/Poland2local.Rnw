...
\begin{itemize}
    ...
    \item Other possible measures could address the relatively large low-wage sector. The recent rise in minimum wages is a step into this direction.
...
<<mean_p1setup, echo=FALSE, results='asis', cache=FALSE>>=
...
levels(meltmean_p1$variable) <- c('Pre-tax factor', 'Pre-tax national', 'Post-tax disposable')
...
plot_meanp1 <- ggplot(meltmean_p1, aes(x=year, y=value, color=variable))+
  ...
  labs(title='Mean Income for P1',
    x = "Year", 
       ...
       caption='EU-SILC 2018, Eurostat 2019') + theme_bw() + 
  scale_color_manual(values=c("#00008B", "#0000FF", "#ADD8E6"))+theme(legend.position = 'bottom', legend.title = element_blank()) #+ scale_fill_manual(name="Income Concepts", labels=c("Control", "Treatment 1", "Treatment 2")) #+scale_linetype_manual(name="Title of legend", values = c('mean_p1$pretaxfacinc', 'mean_p1$pretaxnational', 'mean_p1$posttax'), guide = guide_legend(override.aes = list(color = c('blue', 'red','green')))) 
plot_meanp1
@
...
<<median_p1, echo=FALSE, results='asis', warning=FALSE, cache=TRUE>>=
...
<<figure_medianp1, cache=TRUE, echo=FALSE, fig.cap='Median income in 2015 EUR for the three income concepts over time for P1. The graphs for pre-tax factor income, pre-tax national income and post-tax disposable income are again dark blue, blue and light blue, respectively.', fig.align='center', fig.width=5, fig.height=4>>=
median_p1<-median_p1[,1:4]
meltmedian_p1<-melt(median_p1, id.vars = 'year')
levels(meltmedian_p1$variable) <- c('Pre-tax factor', 'Pre-tax national', 'Post-tax disposable')
...
plot_medianp1 <-  ggplot(meltmedian_p1, aes(x=year, y=value, color=variable))+
  ...
  labs(title='Median Income for P1', x = "Year", 
       y = "Yearly income in 2015 EUR",
       caption='EU-SILC 2018, Eurostat 2019')+theme_bw() + 
  scale_color_manual(values=c("#00008B", "#0000FF", "#ADD8E6"))+theme(legend.position = 'bottom', legend.title = element_blank())
plot_medianp1
@
<<figure_density, cache=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.cap='A depiction of of the distribution post-tax disposable income in 2017. As visible, mean and median are very close to each other.', fig.align='center', fig.width=5, fig.height=4>>=
...
<<figure_lorenz, cache=TRUE, echo=FALSE, fig.cap='Lorenz curve for pre-tax factor income in 2017.', fig.align='center', fig.width=5, fig.height=4>>=
...
<<figure_ginip1, cache=TRUE, echo=FALSE, fig.cap='The development of the Gini coefficient for all three income concepts over time for P1. As everywhere, pre-tax factor income is again dark blue, pre-tax national income is blue and post-tax disposable income is light blue.', fig.align='center', fig.width=5, fig.height=4>>=
gini_p1<-gini_p1[,1:6]
gini_p1<-gini_p1[,-3]
gini_p1<-gini_p1[,-4]
meltgini_p1<-melt(gini_p1, id.vars = 'year')
levels(meltgini_p1$variable) <- c('Pre-tax factor', 'Pre-tax national', 'Post-tax disposable')
...
<<s80_s20p1, cache=TRUE, echo=FALSE, results='asis', warning=FALSE>>=
...
<<figure_s80_s20p1, cache=TRUE, eval=TRUE, echo=FALSE, fig.cap='The development of the S80/S20 ratio for all three income concepts over time for P1. Pre-tax factor income is dark blue, pre-tax national income is blue and post-tax disposable income is light blue.', fig.align='center', fig.width=5, fig.height=4, fig.pos='H'>>=
s80_s20p1<-s80_s20p1[,-6]
s80_s20p1<-s80_s20p1[,-3]
s80_s20p1melt<-melt(s80_s20p1, id.vars = 'year')
levels(s80_s20p1melt$variable) <- c('Pre-tax factor', 'Pre-tax national', 'Post-tax disposable')
...
<<top10p1, echo=FALSE, cache=TRUE, results='asis', warning=FALSE>>=
...
top10_P1[1:3] <- top10_P1[1:3]*100


top10p1year <- top10_P1
top10p1year$year <- c(2005:2017)
...
<<figure_top10p1, cache=TRUE, eval=TRUE, echo=FALSE, fig.cap='Depiction of the development of the total income share of the top 10 percent for all three income concepts over time for P1. Pre-tax factor income is dark blue, pre-tax national income is blue and post-tax disposable income is light blue.', fig.align='center', fig.width=5, fig.height=4, fig.pos='H'>>=
top10p1yearmelt<-melt(top10p1year, id.vars = 'year')
levels(top10p1yearmelt$variable) <- c('Pre-tax factor', 'Pre-tax national', 'Post-tax disposable')
...
<<ols1, results='asis', echo=FALSE, cache=TRUE, error=FALSE, warning=FALSE, message=FALSE>>=
...
@
Our analysis shows indeed a very large university wage premium, especially for men: In 2005, a man with tertiary education had a pre-tax factor income \Sexpr{round(sols2005PTFI$coefficients[9,1],2)} EUR higher than a man of the same age with no education at all, and still \Sexpr{round(sols2005PTFI$coefficients[9,1]-sols2005PTFI$coefficients[8,1],2)} EUR more than a man of same age with a post-secondary education. For women, the respective values are \Sexpr{round(sols2005PTFI$coefficients[9,1]+sols2005PTFI$coefficients[14,1],2)} EUR and \Sexpr{round((sols2005PTFI$coefficients[9,1]+sols2005PTFI$coefficients[14,1])-(sols2005PTFI$coefficients[8,1]+sols2005PTFI$coefficients[13,1]),2)} EUR. The redistributive system is able to temper these extreme differences among educational groups (the respective values regarding the post-tax disposable income for men are \Sexpr{round(sols2005PTDI$coefficients[9,1],2)} EUR and \Sexpr{round(sols2005PTDI$coefficients[9,1]-sols2005PTDI$coefficients[8,1],2)} EUR, and for women \Sexpr{round(sols2005PTDI$coefficients[9,1]+sols2005PTDI$coefficients[14,1],2)} EUR and \Sexpr{round((sols2005PTDI$coefficients[9,1]+sols2005PTDI$coefficients[14,1])-(sols2005PTDI$coefficients[8,1]+sols2005PTDI$coefficients[13,1]),2)} EUR), but they remain high. In 2013, we see that the university wage premium relative to the intercept has somehow decreased compared to 2005 both for men (pre-tac factor income: \Sexpr{round(sols2013PTFI$coefficients[9,1],2)} EUR, post-tax disposable income: \Sexpr{round(sols2013PTDI$coefficients[9,1],2)} EUR) and women (pre-tax factor income: \Sexpr{round(sols2013PTFI$coefficients[9,1]+sols2013PTFI$coefficients[14,1],2)} EUR, post-tax disposable income: \Sexpr{round(sols2013PTDI$coefficients[9,1]+sols2013PTDI$coefficients[14,1],2)} EUR). The existence of this high differences in wages, however, can perhaps be explained with the poor redistributive performance of the Polish income tax system, which only has one progression step \parencite{pwc}.

When it comes to gender income equality, we see that in 2005, women had on average \Sexpr{round(sols2005PTFI$coefficients[4,1],2)} EUR more in pre-tax factor income than men of the same age and education in the intercept category, \Sexpr{(-1)*(round(sols2005PTFI$coefficients[4,1]+sols2005PTFI$coefficients[13,1],2))} EUR less if both were on post-secondary level and \Sexpr{(-1)*(round(sols2005PTFI$coefficients[4,1]+sols2005PTFI$coefficients[14,1],2))} EUR less if both had studied at university. The respective amounts for post-tax disposable income are \Sexpr{(-1)*(round(sols2005PTDI$coefficients[4,1],2))} EUR less, \Sexpr{(-1)*(round(sols2005PTDI$coefficients[4,1]+sols2005PTDI$coefficients[13,1],2))} EUR less and \Sexpr{(-1)*(round(sols2005PTDI$coefficients[4,1]+sols2005PTDI$coefficients[14,1],2))} EUR less. For both income concepts, we observe a rise in gender inequality along education, and only a negligible effect of the redistributive system. In 2013, women had on average \Sexpr{(-1)*(round(sols2013PTFI$coefficients[4,1],2))} EUR less in pre-tax factor income than men of the same age and education in the intercept category, \Sexpr{(-1)*(round(sols2013PTFI$coefficients[4,1]+sols2013PTFI$coefficients[13,1],2))} EUR less if both were on post-secondary level and \Sexpr{(-1)*(round(sols2013PTFI$coefficients[4,1]+sols2013PTFI$coefficients[14,1],2))} EUR less on the tertiary level. Here, the respective amounts for post-tax disposable income are \Sexpr{(-1)*(round(sols2013PTDI$coefficients[4,1],2))} EUR less, \Sexpr{(-1)*(round(sols2013PTDI$coefficients[4,1]+sols2013PTDI$coefficients[13,1],2))} EUR less and \Sexpr{(-1)*(round(sols2013PTDI$coefficients[4,1]+sols2013PTDI$coefficients[14,1],2))} EUR less. We thus can see that in the time between 2005 and 2013, the gender income gap has widened, as it is higher in post-tax disposable income for all three mentioned education levels.          

Of course, we also have to keep in mind that for P2 there is no individual taxation of incomes, presenting these premia in a very drastical light. Nonetheless, it has become clear that income inequality in Poland is indeed driven by gender and educational premia. Regarding the latter, this is in line with \cite{gomulka2016poland}, who states:
...
\section{Appendix}
<<mean_p2, echo=FALSE, cache=TRUE, results='asis'>>=
...
<<figure_meanp2, cache=TRUE, echo=FALSE, fig.cap='Mean income for all three income concepts over time for P2. As in Figure 1, pre-tax factor income is again dark blue, pre-tax national income is blue and post-tax disposable income is light blue.', fig.align='center', fig.width=5, fig.height=4, fig.pos='H'>>=
mean_p2<-mean_p2[,1:4]
meltmean_p2<-melt(mean_p2, id.vars = 'year')
levels(meltmean_p2$variable) <- c('Pre-tax factor', 'Pre-tax national', 'Post-tax disposable')
...
<<median_p2, echo=FALSE, cache=TRUE, results='asis', warning=FALSE>>=
...
<<figure_medianp2, echo=FALSE, cache=TRUE, fig.cap='Median income in 2015 EUR for the three income concepts over time for P2. Pre-tax factor income is again dark blue, pre-tax national income is blue and post-tax disposable income is again light blue.', fig.align='center', fig.width=5, fig.height=4, fig.pos='H'>>=
median_p2<-median_p2[,1:4]
meltmedian_p2<-melt(median_p2, id.vars = 'year')
levels(meltmedian_p2$variable) <- c('Pre-tax factor', 'Pre-tax national', 'Post-tax disposable')
...
<<gini_p2, echo=FALSE, cache=TRUE, results='asis', warning=FALSE>>=
#we need to calculate the gini several times, because svygini apparently allows only for one variable
...
<<figure_ginip2, eval=TRUE, cache=TRUE, echo=FALSE, fig.cap='The development of the Gini coefficient for all three income concepts over time for P2. Pre-tax factor income is again dark blue, pre-tax national income is blue and post-tax disposable income is light blue.', fig.align='center', fig.width=5, fig.height=4, fig.pos='H'>>=
gini_p2<-gini_p2[,1:6]
gini_p2<-gini_p2[,-3]
gini_p2<-gini_p2[,-4]
meltgini_p2<-melt(gini_p2, id.vars = 'year')
levels(meltgini_p2$variable) <- c('Pre-tax factor', 'Pre-tax national', 'Post-tax disposable')
...
<<s80_s20p2, echo=FALSE, cache=TRUE, results='asis', warning=FALSE>>=
#as with the gini, we have to calculate the s80/s20 several times, because the function only allows for one variable
...
<<figure_s80_s20p2, eval=TRUE, cache=TRUE, echo=FALSE, fig.cap='The development of the S80/S20 ratio for all three income concepts over time for P2. Pre-tax factor income is dark blue, pre-tax national income is blue and post-tax disposable income is light blue.', fig.align='center', fig.width=5, fig.height=4, fig.pos='H'>>=
s80_s20p2<-s80_s20p2[,-6]
s80_s20p2<-s80_s20p2[,-3]
s80_s20p2melt<-melt(s80_s20p2, id.vars = 'year')
levels(s80_s20p2melt$variable) <- c('Pre-tax factor', 'Pre-tax national', 'Post-tax disposable')
...
<<top10p2, echo=FALSE, results='asis', cache=TRUE, warning=FALSE>>=
#create cut off
...
colnames(top10_P2)[1:3] <- c("PTFI", "PTNI", "PTDI")
top10_P2[1:3] <- top10_P2[1:3]*100


top10p2year <- top10_P2
top10p2year$year <- c(2005:2017)

#####latex part

names(top10_P2)<-c('Pre-tax factor', 'Pre-tax national', 'Post-tax disposable')
print.xtable(xtable(top10_P2,align='lccc', caption='The total-income share of the top 10 percent over time for all three income concepts for P2 (EU-SILC 2018, Eurostat 2019).', digits=2, label='top10p2'), booktabs = TRUE)

@


<<figure_top10p2, eval=TRUE, cache=TRUE, echo=FALSE, fig.cap='Depiction of the development of the total income share of the top 10 percent for all three income concepts over time for P2. Pre-tax factor income is dark blue, pre-tax national income is blue and post-tax disposable income is light blue.', fig.align='center', fig.width=5, fig.height=4, fig.pos='H'>>=
top10p2yearmelt<-melt(top10p2year, id.vars = 'year')
levels(top10p2yearmelt$variable) <- c('Pre-tax factor', 'Pre-tax national', 'Post-tax disposable')

plot_top10p2 <-  ggplot(top10p2yearmelt, aes(x=year, y=value, color=variable))+
  geom_hline(yintercept=30, color = "black", size=0.25)+
  geom_line(aes(color=variable), size=0.5) +
  geom_point(aes(color=variable, shape=variable), size=1) +
