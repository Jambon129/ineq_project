...
\hlkwd{library}\hlstd{(convey)}
\hlkwd{library}\hlstd{(scales)}
\hlkwd{load}\hlstd{(}\hlstr{'../../data/P1.rda'}\hlstd{)}
\hlkwd{load}\hlstd{(}\hlstr{'../../data/P2.rda'}\hlstd{)}


\hlcom{#convert NA to zero}
\hlstd{P1[}\hlkwd{is.na}\hlstd{(P1)]} \hlkwb{<-} \hlnum{0}
\hlstd{P2[}\hlkwd{is.na}\hlstd{(P2)]} \hlkwb{<-} \hlnum{0}

\hlcom{#controll for inflation by division of the income aggregates by cPI}

\hlstd{P1}\hlkwb{<-}\hlkwd{mutate}\hlstd{(P1,} \hlkwc{PTFI_clean}\hlstd{=pretaxfacinc}\hlopt{*}\hlnum{100}\hlopt{/}\hlstd{CPI)}
\hlstd{P1}\hlkwb{<-}\hlkwd{mutate}\hlstd{(P1,} \hlkwc{PTNI_clean}\hlstd{=pretaxnational}\hlopt{*}\hlnum{100}\hlopt{/}\hlstd{CPI)}
\hlstd{P1}\hlkwb{<-}\hlkwd{mutate}\hlstd{(P1,} \hlkwc{PTDI_clean}\hlstd{=posttax}\hlopt{*}\hlnum{100}\hlopt{/}\hlstd{CPI)}

\hlstd{P2}\hlkwb{<-}\hlkwd{mutate}\hlstd{(P2,} \hlkwc{PTFI_clean}\hlstd{=pretaxfactor}\hlopt{*}\hlnum{100}\hlopt{/}\hlstd{CPI)}
\hlstd{P2}\hlkwb{<-}\hlkwd{mutate}\hlstd{(P2,} \hlkwc{PTNI_clean}\hlstd{=pretaxnational}\hlopt{*}\hlnum{100}\hlopt{/}\hlstd{CPI)}
\hlstd{P2}\hlkwb{<-}\hlkwd{mutate}\hlstd{(P2,} \hlkwc{PTDI_clean}\hlstd{=posttax}\hlopt{*}\hlnum{100}\hlopt{/}\hlstd{CPI)}

\hlstd{P1} \hlkwb{<-} \hlstd{P1} \hlopt{%>%} \hlkwd{mutate}\hlstd{(}\hlkwc{id_h}\hlstd{=}\hlkwd{paste0}\hlstd{(persID))}
\hlstd{P2} \hlkwb{<-} \hlstd{P2} \hlopt{%>%} \hlkwd{mutate}\hlstd{(}\hlkwc{id_h}\hlstd{=}\hlkwd{paste0}\hlstd{(persID))}

\hlstd{P1.svy} \hlkwb{<-} \hlkwd{svydesign}\hlstd{(}\hlkwc{ids}\hlstd{=}\hlopt{~}\hlstd{id_h,}
                         \hlkwc{weights}\hlstd{=}\hlopt{~}\hlstd{perswght,}
                         \hlkwc{data}\hlstd{=P1)} \hlopt{%>%} \hlkwd{convey_prep}\hlstd{()}

\hlstd{P2.svy} \hlkwb{<-} \hlkwd{svydesign}\hlstd{(}\hlkwc{ids}\hlstd{=}\hlopt{~}\hlstd{id_h,}
                         \hlkwc{weights}\hlstd{=}\hlopt{~}\hlstd{perswght,}
                         \hlkwc{data}\hlstd{=P2)} \hlopt{%>%} \hlkwd{convey_prep}\hlstd{()}
\end{alltt}
\end{kframe}
\end{knitrout}
...
% Date and time: Do., Jän 10, 2019 - 21:04:10
...
% Table created by stargazer v.5.2.2 by Marek Hlavac, Harvard University. E-mail: hlavac at fas.harvard.edu
% Date and time: Do., Jän 10, 2019 - 21:04:12
\begin{table}[H] \centering 
